...
`bin-v0.5.0` 发布版的每个压缩包都包含四个可执行文件（`cgraph`、`graphd`、`cgraph-client` 和 `cgraph-mcp`）：
...
| macOS | Apple 芯片 / arm64 | [`cgraph-macos-arm64.tar.gz`](https://github.com/Nxtsoft/CGraph/releases/download/bin-v0.5.0/cgraph-macos-arm64.tar.gz) |

用 `uname -s` 和 `uname -m` 选择压缩包。快速开始把带版本的文件安装到 `~/.local/lib/cgraph/bin-v0.5.0`，并在 `~/.local/bin` 中创建稳定的软链接；如有需要，请把该目录加入 `PATH`。MCP 客户端配置应使用带版本的绝对路径，因为客户端未必继承 shell 的 `PATH`。
