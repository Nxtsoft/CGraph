...
## Purpose
Discover the wire contracts a repository provides from its own source, so services can be joined by contract identity without a hand-written seam spec. This slice covers HTTP endpoints served by JavaScript/TypeScript router chains and Next.js route files.
